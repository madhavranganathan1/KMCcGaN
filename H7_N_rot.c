//Jan 22, 2022 : code for diffusion of N atom at  height multiple of 7, 15, 23.... Each N atom will have only two sites for rotation which depands on the position of Ga bond to which N atom is linked.


#include "variables.h"

int H7_N_rot(int X,int Y)           
{	
     ...
     while(site > 0){
                     ...
                     for (i=0; i<site; i++){
                               if(((i*randiff) < Rdir) && (Rdir < ((i+1)*randiff))){
                                    ...
                                    if     ((Box[Xp1][Y]  [H[X][Y]+1]) ==1) { if((i==3||i==4)) { Prob = Prob_NRot; } else { Prob = Prob_NHop; }}					   
                                    else if((Box[Xm1][Ym1][H[X][Y]+1]) ==1) { if((i==5||i==6)) { Prob = Prob_NRot; } else { Prob = Prob_NHop; }}				   
                                    else if((Box[Xm1][Yp1][H[X][Y]+1]) ==1) { if((i==7||i==8)) { Prob = Prob_NRot; } else { Prob = Prob_NHop; }}
				    
                                    double Racc = gsl_rng_uniform_pos(racc);
                                    if(Racc < Prob){
                                                    if(abs(Prob - Prob_NRot) < 0.0000001){
                                                       if((Box[NX_H4[i]][NY_H4[i]][H[NX_H4[i]][NY_H4[i]]]) ==3) {
                                                           if((H[NX_H4[i]][NY_H4[i]] - H[X][Y]) ==0){
                                                               ...
                                                               if(Neighb!=0){
                                                                ...
                                                                if(( H[X][Y]-H[NX_H4[i]][NY_H4[i]]) ==3){
                                                                     Neighb = countNeighb(NX_H4[i],NY_H4[i],H[NX_H4[i]][NY_H4[i]]);
                                                                     if(Neighb!=0){
                                                                                   H[NX_H4[i]][NY_H4[i]] = H[NX_H4[i]][NY_H4[i]]  + 3 ;
                                                                                   Box[NX_H4[i]][NY_H4[i]][H[NX_H4[i]][NY_H4[i]]] = 2 ;
                                                                                   Box[X][Y][H[X][Y]] = 0;      H[X][Y] = H[X][Y] - 3 ;
                                                                                   N_move[cntGa]++ ;  stable_N(NX_H4[i], NY_H4[i]);  nNrot++; break;}}}						    								
						    }
                                                         ...
                                                         if((Box[NX_H4[i]][NY_H4[i]][H[NX_H4[i]][NY_H4[i]]]) ==3) {
                                                             ...
                                                             if(Neighb!=0){
                                                                  ...
                                                                  if(Neighb!=0){
                                                                                ...
                                                                                Box[NX_H4[i]][NY_H4[i]][H[NX_H4[i]][NY_H4[i]]] = 2 ;
                                                                                Box[X][Y][H[X][Y]] = 0;      H[X][Y] = H[X][Y] - 3 ;
                                                                                N_move[cntGa]++ ;  stable_N(NX_H4[i], NY_H4[i]);  nNhop++; break;}}}}
                     ...
                     if(Box[X][Y][H[X][Y]] == 1){			     
                                                 for(c=0; c<3; c++){							 
                                                          if((Box[NX_H4[c]][NY_H4[c]][H[NX_H4[c]][NY_H4[c]]]) == 2){
                                                              if((H[NX_H4[c]][NY_H4[c]] - H[X][Y])>0){  
							            NN = countNeighb(NX_H4[c],NY_H4[c],(H[NX_H4[c]][NY_H4[c]]-3));
                                                                    if(NN == 0){
                                                                                H[X][Y] = H[X][Y]  + 3;
                                                                                Box[X][Y][H[X][Y]] = 2;
                                                                                Box[NX_H4[c]][NY_H4[c]][H[NX_H4[c]][NY_H4[c]]] = 0;
                                                                                H[NX_H4[c]][NY_H4[c]] = H[NX_H4[c]][NY_H4[c]]  - 3;
                                                                                side_move_Ga++; break; }}}}
                                                break; }
